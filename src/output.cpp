...
void Output::WriteHistory(Integration *solver, Structure *structure, ofstream *outputfile, const double &time)
{
    if (structure->GetnDof() == 1)
    {
        ...
        std::cout << time << "\t" << (*(solver->GetDisp()))[0] << "\t" << (*(solver->GetVel()))[0] << "\t" << (*(solver->GetAcc()))[0] << std::endl;
    ...
    else if (structure->GetnDof() == 2)
    {
        ...
        std::cout << time << "\t" << (*(solver->GetDisp()))[0] << "\t" << (*(solver->GetDisp()))[1] << "\t" << (*(solver->GetVel()))[0] << "\t" << (*(solver->GetVel()))[1] << "\t" << (*(solver->GetAcc()))[0] << "\t" << (*(solver->GetAcc()))[1] << std::endl;
...
void Output::WriteRestart(Integration *solver, Structure *structure)
{
    ...
    RestartFile.open("Nat_solution_restart.out", std::ios::out);

    if (structure->GetnDof() == 1)
    {
        RestartFile << "\"Displacement\""
                    << "\t"
                    << "\"Velocity\""
                    << "\t"
                    << "\"Acceleration\""
        ...
        RestartFile << (*(solver->GetDisp()))[0] << "\t" << (*(solver->GetVel()))[0] << "\t" << (*(solver->GetAcc()))[0] << "\t" << (*(solver->GetAccVar()))[0] << std::endl;
    }
    else if (structure->GetnDof() == 2)
    {
        RestartFile << "\"Displacement 1\""
                    << "\t"
                    << "\"Displacement 2\""
                    << "\t"
                    << "\"Velocity 1\""
        ...
        RestartFile << time << "\t" << (*(solver->GetDisp()))[0] << "\t" << (*(solver->GetDisp()))[1] << "\t" << (*(solver->GetVel()))[0] << "\t" << (*(solver->GetVel()))[1] << "\t" << (*(solver->GetAcc()))[0] << "\t" << (*(solver->GetAcc()))[1] << "\t" << (*(solver->GetAccVar()))[0] << "\t" << (*(solver->GetAccVar()))[1] << std::endl;
    }
}
...
void Output::WriteStaticSolution(Config *config, Integration *solver, Structure *structure, ofstream *outputfile)
{
    if (structure->GetnDof() == 1)
    {
        std::cout << "Static displacement is : " << (*(solver->GetDisp()))[0] << " [m]" << std::endl;
        std::cout << "Writing displacement into a solution file" << std::endl;
    ...
    {
        std::cout << "Plunging displacement is :" << (*(solver->GetDisp()))[0] << " [m]" << std::endl;
        std::cout << "Pitching rotation is :" << (*(solver->GetDisp()))[1] << " [rad]" << std::endl;
        std::cout << "Writing displacement into a solution file" << std::endl;
